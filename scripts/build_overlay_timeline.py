#!/usr/bin/env python3
"""Build V2 per-scene overlay timelines."""
...
try:
    from video_pipeline_v2 import DEFAULT_RENDER_PLAN_DIR, build_scene_overlay_timeline_payload, dump_json, find_manifest_for_episode
except ModuleNotFoundError:
    from scripts.video_pipeline_v2 import DEFAULT_RENDER_PLAN_DIR, build_scene_overlay_timeline_payload, dump_json, find_manifest_for_episode
...
def main() -> int:
    ...
    for events_path in sorted(render_dir.glob("scene_*.events.json")):
        ...
        payload = build_scene_overlay_timeline_payload(events_path, audio_plan_path, manifest_path)
        output_path = render_dir / events_path.name.replace(".events.json", ".overlay_timeline.json")
